...
class DataList{
	...
	// get post and get variables
		...
		$Embedded = intval($_REQUEST['Embedded'] ?? 0);
		$AutoClose = intval($_REQUEST['AutoClose'] ?? 0);
		...
		$SearchString = (function_exists('get_magic_quotes_gpc') && get_magic_quotes_gpc() ? stripslashes($_REQUEST['SearchString'] ?? '') : ($_REQUEST['SearchString'] ?? ''));
		$CSV_x = $_REQUEST["CSV_x"] ?? '';
		$Print_x = $_REQUEST['Print_x'] ?? '';
		$PrintTV = $_REQUEST['PrintTV'] ?? '';
		$PrintDV = $_REQUEST['PrintDV'] ?? '';
		$SelectedID = (function_exists('get_magic_quotes_gpc') && get_magic_quotes_gpc() ? stripslashes($_REQUEST['SelectedID'] ?? '') : ($_REQUEST['SelectedID'] ?? ''));
