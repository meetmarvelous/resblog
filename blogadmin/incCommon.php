<?php
	...
	*/
	...
	function getTableList($skip_authentication = false){
		...
		if(is_array($arrTables)){
			foreach($arrTables as $tn => $tc){
				...
				if($arrPerm && isset($arrPerm[0]) && $arrPerm[0]){
	...
	function getTablePermissions($tn){
		...
		return isset($table_permissions[$tn]) ? $table_permissions[$tn] : false;
	}
	...
	function showNotifications($msg = '', $class = '', $fadeout = true){
		...
		if(!$msg){ // if no msg, use url to detect message to display
			if(isset($_REQUEST['record-added-ok']) && $_REQUEST['record-added-ok'] != ''){
				$msg = $Translation['new record saved'];
				$class = 'alert-success';
			}elseif(isset($_REQUEST['record-added-error']) && $_REQUEST['record-added-error'] != ''){
				$msg = $Translation['Couldn\'t save the new record'];
				$class = 'alert-danger';
				$fadeout = false;
			}elseif(isset($_REQUEST['record-updated-ok']) && $_REQUEST['record-updated-ok'] != ''){
				$msg = $Translation['record updated'];
				$class = 'alert-success';
			}elseif(isset($_REQUEST['record-updated-error']) && $_REQUEST['record-updated-error'] != ''){
				$msg = $Translation['Couldn\'t save changes to the record'];
				$class = 'alert-danger';
				$fadeout = false;
			}elseif(isset($_REQUEST['record-deleted-ok']) && $_REQUEST['record-deleted-ok'] != ''){
				$msg = $Translation['The record has been deleted successfully'];
				$class = 'alert-success';
				$fadeout = false;
			}elseif(isset($_REQUEST['record-deleted-error']) && $_REQUEST['record-deleted-error'] != ''){
